...
export default {
  ...
  methods: {
    ...
    async addFollow () {
      ...
      await axios
        .get(`http://parchegramtest.com/api/Follow/Add/${this.$session.get('nameUser')}/${this.idUserFollowing}`)
        .then(result => {
    ...
    async deleteFollow () {
      this.userFollow = false
      await axios
        .get(`http://parchegramtest.com/api/Follow/Delete/${this.$session.get('nameUser')}/${this.idUserFollowing}`)
        .then(result => {
          if (result.data.success !== 1) {
            this.userFollow = true
