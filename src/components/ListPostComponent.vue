  ...
  methods: {
    async loadMore () {
      ...
      if (!this.full && !this.executing) {
        ...
        let url = ''
        if (this.fromProfile) {
          url = `http://parchegramtest.com/api/Post/GetPosts/${this.nameUser}/${this.page}/${this.idTypePost}/${this.$session.get('nameUser')}`
        } else {
          url = `http://parchegramtest.com/api/Post/GetPosts/${this.$session.get('nameUser')}/${this.page}`
        }
        await axios
          .get(url)
          .then(result => {
